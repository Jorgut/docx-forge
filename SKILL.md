---
name: docx-forge
description: "Professional Word document creation, editing, and analysis. Generate .docx files programmatically, edit existing documents via XML, handle tracked changes, comments, tables, images, and complex formatting. Works with docx (npm), pandoc, and raw OOXML for full control."
license: MIT
---

# docx-forge — Word Document Engineering

## Overview

A `.docx` file is a ZIP archive containing XML files. This skill covers two approaches:

| Task | Approach |
|------|----------|
| **Create** new documents | Use `docx` (npm) — see Creating New Documents below |
| **Edit** existing documents | Unpack → edit XML → repack |
| **Read** content | `pandoc -t markdown file.docx` |
| **Accept tracked changes** | `pandoc --track-changes=accept file.docx -o clean.docx` |

### Read Content

```bash
# Text extraction with tracked changes visible
...
```bash
pandoc document.docx -o output.pdf  # requires --pdf-engine
# or use LibreOffice directly:
...
After creating the file, verify it opens correctly. For validation:

```bash
# Check it's a valid ZIP with correct OOXML structure
unzip -t output.docx

# Or use a validation library
npm install -g ooxml-validator
ooxml-validator output.docx
```

### Page Size

```javascript
// docx defaults to A4 — set explicitly for US documents
sections: [{
  properties: {
    page: {
      size: {
...
**Landscape:** docx swaps width/height internally, so pass portrait dimensions and let it handle the swap:
...
- Full-width tables = page width − left/right margins
...
### Critical Rules for docx

- **Set page size explicitly** — defaults to A4; use 12240×15840 DXA for US Letter
- **Landscape: pass portrait dimensions** — docx swaps internally
- **Never use `\n`** — use separate Paragraph elements
- **Never use unicode bullets** — use `LevelFormat.BULLET` with numbering config
- **PageBreak must be in Paragraph** — standalone creates invalid XML
- **ImageRun requires `type`** — always specify format
- **Always use `WidthType.DXA` for tables** — never PERCENTAGE (breaks Google Docs)
- **Tables need dual widths** — `columnWidths` + cell `width`, both matching
- **Table width = sum of columnWidths**
- **Always add cell margins** — `{ top: 80, bottom: 80, left: 120, right: 120 }`
- **Use `ShadingType.CLEAR`** — never SOLID for table shading
- **Never use tables as dividers** — cells have minimum height; use `border-bottom` on Paragraph instead
...
Process: **Unpack → Edit XML → Repack**

### Unpack

```bash
unzip document.docx -d unpacked/
# XML files are in unpacked/word/
```
...
```xml
<!-- Professional typography entities -->
...
```bash
cd unpacked
zip -r ../output.docx . -x ".*" -x "__MACOSX"
```

For proper OOXML packaging (correct content types and relationships), use a library or reference the OOXML standard.
...
Comment markers are siblings of `<w:r>`, never inside `<w:r>`.
